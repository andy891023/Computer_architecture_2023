...
class hw2 extends AnyFlatSpec with ChiselScalatestTester {
  behavior.of("Single Cycle CPU")
  it should "FP32 Operations by Applying FP32 to Bfloat16 Conversion Algorithm" in {
    test(new TestTopModule("hw2.asmbin")).withAnnotations(TestAnnotations.annos) {  c =>
      ...
      c.clock.step()
      c.io.mem_debug_read_data.expect(66912256L.U(32.W))
    }
  }
}
